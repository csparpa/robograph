...
def sort_and_unique(input_file_path, output_file_path):
    ...
    to_string_list = apply.Apply(lambda c: c.split('\n'), name="to_string_list")
    ...
    uniquer = apply.Apply(remove_duplicates_from, name='uniquer')
    sorter = apply.Apply(lambda unsorted: sorted(unsorted), name="sorter")
    to_string = apply.Apply(lambda token_list: '\n'.join(token_list),
                            name='to_string')
